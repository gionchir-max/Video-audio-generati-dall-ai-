#!/usr/bin/env node
// Auto-mode: genera N clip agnelli su meta.ai via CDP del browser MCP
// Stesso flow di _run-berlusconi.mjs, slug = agnelli
...
async function main() {
  ...
  for (let clipNum = 1; clipNum <= TOTAL_CLIPS; clipNum++) {
    ...
    const outPath = path.join(OUT_DIR, String(clipNum).padStart(3, '0') + '.mp4');
